...
- **候補モデルを自動比較** — 線形回帰(Ridge)・多項式Ridge・LightGBM(GBDT/RandomForest/ExtraTrees)・
  ...
  最も精度の良いものを自動選択
- **3つの学習モード**
  - **瞬速** — 学習行数を上限キャップしてお急ぎよりさらに短時間で結果を得る(データが多い場合)
...
[CC BY-NC 4.0](LICENSE)（表示 - 非営利）

### ✅ 教育目的での利用（自由）

以下のような用途は許可なく自由にご利用いただけます:
- 学校・大学での学習・研究
- 学習教材・チュートリアルの作成
- 個人の学習・研究目的での利用・改変
...
以下のような用途は商用利用とみなし、事前の許可が必要です:
- SaaS化（月額/年額課金でのサービス提供）
- API化しての販売
- 企業研修・コンサルティングでの利用
- 本ソフトウェア（またはその改変版）を組み込んだ製品・サービスの販売

商用利用をご検討の場合は、事前に下記連絡先までご相談ください。

### 表記方法の例

本ソフトウェア（またはその一部）を利用・再配布する際は、以下のように表示してください:

```
Powered by T-regressor
